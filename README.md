...
## Javascript Assignments  [![Build Status](https://travis-ci.org/dzmitrybudzko/rolling-scopes-js-assignments.svg?branch=master)](https://travis-ci.org/dzmitrybudzko/rolling-scopes-js-assignments)

Yet another javascript assignments. There are a lot of interactive javascript resources for beginners, but most of them are online and do not cover the modern programming workflow. There are some excelent training resources on github (https://github.com/rmurphey/js-assessment, https://github.com/mrdavidlaing/javascript-koans etc) but they are not exactly simulate the everyday programming process. So the motivation of this project is to show TDD process in the wild to the begginers. Assignment tests are implemented in various ways to feel a difference and gain the experience what manner is good, what is bad and what is ugly.
...
* Edit local README.md file and update all links (just replace all occurrences of `'dzmitrybudzko'` with your account name).
...
```
* Open https://github.com/dzmitrybudzko/rolling-scopes-js-assignments and test the build icon. Now it will run all tests and update status once you push changes to github. Keep this icon green!
...
* Push your updates to github server and check if tests passed on [travis-ci](https://travis-ci.org/dzmitrybudzko/rolling-scopes-js-assignments/builds).
